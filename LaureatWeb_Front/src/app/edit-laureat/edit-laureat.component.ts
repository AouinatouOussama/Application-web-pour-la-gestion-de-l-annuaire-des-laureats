  ...
  modifierAvancement() {
    ...
    this.httpClient.get("http://localhost:9090/requestAny/select%20max(id)%20from%20avancement").subscribe( dataa => {
      ...
      this.httpClient.get("http://localhost:9090/requestAny/" +
        ...
        "VALUES%20" +
        "("+ (Number((dataa as any).features[0].max) + 1).toString()  +",'" + formatDate(new Date(), 'yyyy-MM-dd', 'en').toString() + "','" + this.motif + "',%20" + this.dernierAvancement +  ",%20'"+ this.etat +"')").subscribe( data => {
